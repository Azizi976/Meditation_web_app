...
export default function Register() {
  ...
  const [form, setForm]   = useState({ email: '', password: '', displayName: '', language: lang });
    ...
    <div className="flex items-center justify-center min-h-[70vh]">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-display font-semibold text-slate-700">{t('auth.register_title')}</h1>
          <p className="text-sm text-slate-500 mt-1">{t('auth.register_subtitle')}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-1.5">{t('auth.display_name')}</label>
            <input
              type="text"
        ...
        <p className="text-center text-sm text-slate-500 mt-6">
          {t('auth.have_account')}{' '}
          <Link to="/login" className="text-lavender-600 font-medium hover:underline">
            {t('auth.sign_in')}
          </Link>
        </p>
      </motion.div>
